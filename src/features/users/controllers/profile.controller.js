...
/*
 * Vercel Functions have a request-body limit below the old 5 MB avatar limit.
 * Keep avatar uploads comfortably under that boundary because this endpoint
 * receives the multipart body through the backend.
 */
...
function cleanProfilePatch(body = {}) {
  ...
  for (const key of PROFILE_FIELDS) {
    ...
    if (key === 'themePreference') {
      if (['light', 'dark'].includes(body[key])) {
        patch[key] = body[key];
      }
...
export async function updateMyProfile(req, res) {
  if (
    ...
    !cleanPublicUrl(req.body.businessWebsite)
  ) {
...
export async function setMyAvatar(req, res) {
  try {
    ...
    const uploaded = await fileStorage.uploadBuffer(
      storePath,
      file.buffer,
      contentType,
      {
    ...
    const oldPath = String(user.avatarPath || '');

    user.avatarUrl = uploaded.url;
    user.avatarPath = uploaded.path || storePath;
    ...
    if (
      oldPath &&
      oldPath !== String(user.avatarPath || '')
    ) {
      try {
        await fileStorage.removePath(oldPath);
      } catch (error) {
        cleanupPending = true;
...
export async function deleteMyAvatar(req, res) {
  try {
    ...
    /*
     * Clear the user record even if no physical file is currently attached.
     */
    user.avatarUrl = '';
    user.avatarPath = '';
    ...
    return res.status(status).json({
      ...
      code: error?.code || 'AVATAR_REMOVE_FAILED',
    });
  }
}